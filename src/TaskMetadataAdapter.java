...
public class TaskMetadataAdapter extends LiteralBasedProvider {

    /**
     * The associated feed to which messages will be forwarded;
     * it is the Data Adapter itself.
     */
    private volatile TaskAdapter chatFeed;
    ...
    private void loadChatFeed() throws CreditsException {
        if (this.chatFeed == null) {
            try {
                ...
                this.chatFeed = TaskAdapter.feedMap.get(this.adapterSetId);
            } catch(Throwable t) {
    ...
    private void handleChatMessage(String[] pieces, String task, String session) throws NotificationException {
        //extract session infos

        if (pieces.length != 2) {
            logger.warn("Wrong message received: " + task);
            throw new NotificationException("Wrong message received");
        }

        Map<String,String> sessionInfo = sessions.get(session);
        if (sessionInfo == null) {
            logger.warn("Message received from non-existent session: " + task);
            throw new NotificationException("Wrong message received");
        }
        ...
        if (pieces[0].equals("TASK")) {
            //and send it to the feed
            if (!this.chatFeed.sendMessage(task)) {
                logger.warn("Wrong message received: " + task);
                throw new NotificationException("Wrong message received");
            }
        } else {
